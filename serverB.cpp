...
int main ()
{
    ...
    int costarray[5]={2,0,0,0,0}; // The array which is going to be transmitted

    fp = fopen(filename1, "r");  // Opening the file 

    if (fp == NULL)
        {
            printf ("Error opening the file ServerB.txt\n\n'");
            exit(EXIT_FAILURE);
        } 
    else 
        {    
        ...
        printf(" \nThe Server B has the following neighbour information: \nNeighbour \t Cost \n");
        ...
        if(connect(socktcp,(struct sockaddr *) &cli_addr,sizeof(cli_addr))<0)
            {
                error(" Server B tried connecting with client but in vain \n");
            }
        ...
        printf(" \nThe Server B finishes sending its neighbor information "
                "to the Client with TCP port number %d and IP address %s  \nFor "
                "this connection with the Client, the Server B"
