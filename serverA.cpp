...
int main ()
{
    ...
    int costarray[5]={1,0,0,0,0}; // The array which is going to be transmitted

    fp = fopen(filename1, "r");  // Opening the file 

    if (fp == NULL)
        {
            printf ("Error opening the file ServerA.txt\n\n'");
            exit(EXIT_FAILURE);
        } 
    else 
        {    
        ...
        printf("\nThe Server A has the following neighbour information:\nNeighbour \t Cost \n");
        ...
        printf(" \nThe Server A finishes sending its neighbor information "
                "to the Client with TCP port number %d and IP address %s  \nFor "
                "this connection with the Client, the Server A"
