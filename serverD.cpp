...
int main ()
{
    ...
    int costarray[5]={4,0,0,0,0}; // The array which is going to be transmitted

    fp = fopen(filename1, "r");  // Opening the file 

    if (fp == NULL)
        {
            printf ("Error opening the file ServerD.txt\n\n'");
            exit(EXIT_FAILURE);
        } 
    else 
        {    
        ...
        printf(" \nThe Server D has the following neighbour information: \nNeighbour \t Cost \n");
        ...
        if(connect(socktcp,(struct sockaddr *) &cli_addr,sizeof(cli_addr))<0)
            {
                error(" Server D tried connecting with client but in vain \n");
            }
        ...
        printf(" \nThe Server D finishes sending its neighbor information "
                "to the Client with TCP port number %d and IP address %s  \nFor "
                "this connection with the Client, the Server D"
